...
<template>
  <AppPwaLayout>
    <template #title>
      <span class="text-base font-semibold text-foreground">Izin</span>
    </template>

    <!-- ── Info Banner ────────────────────────────── -->
    <section class="bg-muted border border-border rounded-xl p-4 mb-4">
      <div class="flex items-start gap-3">
        <div class="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center flex-shrink-0">
          <User class="w-5 h-5 text-foreground" />
        </div>
        <div>
          <p class="text-sm font-semibold text-foreground">Izin Tidak Masuk</p>
    ...
    <section class="bg-white border border-border rounded-xl p-4 shadow-sm mb-4">
      <label class="text-xs font-medium text-muted-foreground block mb-2">Jenis Izin</label>
    ...
    <section class="bg-white border border-border rounded-xl p-4 space-y-4 mb-4 shadow-sm">
      ...
      <!-- Date -->
      <div class="space-y-1">
        <label class="text-xs font-medium text-muted-foreground block">Tanggal Izin</label>
        <div class="relative">
          <input
            v-model="izinDate"
      ...
      <div class="space-y-1">
        <div class="flex justify-between items-center">
          <label class="text-xs font-medium text-muted-foreground">Alasan</label>
          <span class="text-[10px] text-muted">{{ reason.length }}/200</span>
        </div>
        <textarea
          v-model="reason"
          ...
          rows="4"
        />
        <p class="text-[10px] text-muted italic">Tidak wajib untuk Izin Pribadi</p>
      </div>
    ...
    <div class="bg-muted border border-border rounded-xl p-3 flex items-start gap-2 mb-4">
      <User class="w-4 h-4 text-muted-foreground flex-shrink-0 mt-0.5" />
      <p class="text-xs text-muted-foreground">
        <span class="font-semibold text-foreground">Persetujuan: Supervisor langsung</span>
      </p>
    </div>

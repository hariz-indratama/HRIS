...
<template>
  <AppPwaLayout>
    <template #title>
      <span class="text-base font-semibold text-stitch-primary">Pengajuan Lembur</span>
    </template>

    <!-- ── Quota Card ──────────────────────────────── -->
    <section class="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-center gap-3 mb-4">
      <div class="bg-amber-500 text-white w-12 h-12 rounded-full flex items-center justify-center shadow-sm flex-shrink-0">
        <Clock class="w-6 h-6" />
      </div>
      <div class="flex-1">
        <p class="text-[10px] font-medium text-amber-800 uppercase tracking-wider">Total Lembur Bulan Ini</p>
        <div class="flex items-baseline gap-1">
          <span class="text-[32px] font-bold text-amber-900 leading-tight">8 jam</span>
        </div>
        <p class="text-xs text-amber-700 mt-0.5">Sisa batas lembur: 16 jam</p>
      </div>
    </section>
    ...
    <section class="bg-white border border-stitch-outline-variant rounded-xl p-4 space-y-4 mb-4 shadow-sm">
      ...
      <!-- Date -->
      <div class="space-y-1">
        <label class="text-xs font-medium text-stitch-on-surface-variant block">Tanggal Lembur</label>
        <div class="relative">
          <input
            v-model="otDate"
      ...
      <!-- Time Range -->
      <div class="space-y-1">
        <label class="text-xs font-medium text-stitch-on-surface-variant block">Waktu Lembur</label>
        <div class="grid grid-cols-2 gap-2">
          <div class="relative">
            ...
            />
            <Clock class="absolute right-3 top-3 w-4 h-4 text-stitch-outline pointer-events-none" />
          </div>
          <div class="relative">
            <input
              v-model="endTime"
        ...
        <div class="flex items-center gap-2">
          <span class="px-3 py-1 rounded-full bg-amber-50 border border-amber-200 text-xs font-semibold text-amber-700">
            {{ duration }} jam
          </span>
          <p class="text-[10px] text-stitch-on-surface-variant italic">Waktu istirahat tidak dihitung</p>
        </div>
      </div>

      <!-- Break Duration -->
      <div class="space-y-1">
        <label class="text-xs font-medium text-stitch-on-surface-variant block">Istirahat (jika ada)</label>
      ...
      <div class="space-y-1">
        <div class="flex justify-between items-center">
          <label class="text-xs font-medium text-stitch-on-surface-variant">Deskripsi Pekerjaan</label>
          <span class="text-[10px] text-stitch-outline">{{ description.length }}/300</span>
        </div>
        <textarea
          v-model="description"
      ...
      <!-- Compensation Type -->
      <div class="space-y-1">
        <label class="text-xs font-medium text-stitch-on-surface-variant block">Jenis Kompensasi</label>
        <div class="flex gap-2">
          <button
        ...
        <p v-if="compType === 'timeoff'" class="text-[10px] text-stitch-on-surface-variant italic">
          Ganti hari libur
        </p>
      </div>
    ...
    <div class="bg-stitch-surface-container border border-stitch-outline-variant rounded-xl p-3 flex items-start gap-2 mb-4">
      ...
      <p class="text-xs text-stitch-on-surface-variant">
        <span class="font-semibold text-stitch-on-surface">Persetujuan Required:</span> Anda → Supervisor → HR
      </p>
    </div>
